    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? "sidebar-open" : ""}`}>
        ...
        <nav className="nav-list">
          {navItems.map(({ label, icon: Icon, count }) => <button key={label} className={`nav-item ${activeNav === label ? "active" : ""}`} onClick={() => { setActiveNav(label); setMobileNav(false); }}><Icon size={17} /><span>{label}</span>{count !== undefined && <span className="nav-count">{count}</span>}</button>)}
        </nav>
        <div className="nav-section-label nav-section-spaced">Manage</div>
        <nav className="nav-list">
          {[{ label: "Team", icon: UsersRound }, { label: "Knowledge base", icon: BookOpen }, { label: "Settings", icon: Settings }].map(({ label, icon: Icon }) => <button key={label} className={`nav-item ${activeNav === label ? "active" : ""}`} onClick={() => { setActiveNav(label); toast.info(`${label} is coming soon`, { description: "The overview is the focus of this first release." }); setMobileNav(false); }}><Icon size={17} /><span>{label}</span></button>)}
        </nav>
